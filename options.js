...
async function querySecrets(
  ...
  if (!fetchListOfSecretDirs.ok) {
    ...
    notify.error(`Fetching secrets directories at "${storePath}" failed. ${apiResponse.errors.join('. ')}`);
    return;
  }
...
async function logout() {
  ...
  if (vaultToken) {
    try {
      await fetch(
          `${vaultServerAddress}/v1/auth/token/revoke-self`,
          {
            method: 'POST',
            'X-Vault-Token': vaultToken,
            'Content-Type': 'application/json',
...
async function authToVault(
  ...
  if (!apiResponse.ok) {
    ...
    `);
    new Error(`authToVault: ${await apiResponse.text}`);
  }
  ...
  if (authInfo.lease_duration < 86400) {
    await fetch(
        `${vaultServer}/v1/auth/token/renew-self`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
